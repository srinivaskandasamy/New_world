...
class Particle:
    ...
    def dis_Bayes(self,wall_num, index, index_stddev): # The landmark, the position of measurement of 
        index = int(round(index/self.block,0))#+self.length # For random initial condition
    ...
    def dis_prune_bayes(self,wall,min_value,max_value,block,stddev):
        index = int(((min_value/block)+(max_value/block))/2)#+self.length # For random initial condition
        for j in range(int(stddev/2)):
            b = 0 + j * (0.5/(stddev/2))
            if index - j >= 0:
                wall[index-j]*= b
            if index + j <= int(self.length/self.block)-1:
                wall[index+j]*= b
        wall = [k*100/sum(wall) for k in wall] # Scaling parameter=5
    ...
    def hist_prune(self,ppose,npose):
        ...
        for i in range(len(self.w)):
            ...
            if orientation == 0 and xprod_condition < 0:
                ...
                # Time to prune
                prune_wall = self.dis_prune_bayes(self.wall_list[i],min(y),max(y),self.block,2*(max(y)-min(y)))
                ...
                # Time to prune
                prune_wall = self.dis_prune_bayes(self.wall_list[i],min(x),max(x),self.block,2*(max(x)-min(x)))
...
class FastSLAM:
    ...
    def predict(self,control):
        ...
        head_std  = self.control_head_factor * sqrt(head) # To be modified; mirror image of the speed deviation
        poses = []
    ...
    def update_and_compute_weights(self,measurement):
        ...
        for p in self.particles:
            ...
            wall_list.append(lw)
        weights = [float(weights[i])/sum(weights) for i in arange(len(weights))]    
